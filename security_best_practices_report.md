...
- **Evidence:**
  - `get_current_user_id()` returns a constant: `return 1` in `backend/routes/helpers.py:10-12`.
  - State-changing routes use that helper without any auth checks, for example:
    - `POST /lessons/<id>/start` and `/complete` in `backend/routes/lessons.py:168-280`.
    - `POST /progress/<lesson_id>` in `backend/routes/progress.py:137-182`.
    - `POST /vocabulary/<id>/practice` and `/review` in `backend/routes/vocabulary.py:230-275` and `backend/routes/vocabulary.py:395-443`.
- **Impact (local-only):** Acceptable for a single local user.
- **Impact (if exposed):** Any caller can read or mutate user progress and spaced-repetition stats for the shared user.
...
- **Impact (local-only):** Acceptable.
- **Impact (if exposed):** Debugger can enable remote code execution and dev server is not production‑grade.
...
- **Impact (local-only):** Not applicable.
- **Impact (if exposed):** Reduced defense‑in‑depth against XSS/clickjacking.
...
- **Impact (local-only):** Acceptable.
- **Impact (if exposed):** Leaks internal cache details and allows cache modification.
...
- **Impact (local-only):** Low.
- **Impact (if exposed/shared):** Cached user data could persist on shared devices.
