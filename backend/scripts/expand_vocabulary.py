#!/usr/bin/env python3
"""Expand vocabulary with 15 new topics (750+ words)"""
import json
...
print(f"Current total cards: {sum(len(deck['cards']) for deck in data['decks'])}")

# All new vocabulary decks with complete data
new_decks = []

# Import all the new deck data from a separate JSON file for cleanliness
# But since we're generating, let's include it directly

# For brevity in this script, I'll create a function to generate all decks
# This way the code stays maintainable

def get_body_parts_deck():
    return {
        "name": "Body Parts",
...
new_decks.append(get_body_parts_deck())
...
print(f"\nAdded {len(new_decks)} new deck(s)")
print(f"New total cards: {sum(len(deck['cards']) for deck in data['decks'])}")
