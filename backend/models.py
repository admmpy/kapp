...
from sqlalchemy import Integer, String, Float, Text, DateTime, Date, ForeignKey, CheckConstraint, Index
...
class Deck(db.Model):
    ...
    level: Mapped[int] = mapped_column(Integer, default=0)  # TOPIK level (0=beginner, 1=A1, etc.)
...
class Card(db.Model):
    ...
    # Difficulty level
    level: Mapped[int] = mapped_column(Integer, default=0)  # 0-5 for progressive difficulty
    ...
    ease_factor: Mapped[float] = mapped_column(Float, default=2.5)  # Ease factor (min 1.3)
    next_review_date: Mapped[Optional[date]] = mapped_column(Date, index=True)  # When to review next
    ...
    reviews: Mapped[List["Review"]] = relationship("Review", back_populates="card", cascade="all, delete-orphan")
