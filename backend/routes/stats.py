...
def get_stats():
    ...
    try:
        ...
        cards_due_today = db.session.execute(
            select(func.count())
            .select_from(Card)
            .where((Card.next_review_date == None) | (Card.next_review_date <= today))
        ).scalar()
        ...
        cards_reviewed_today = db.session.execute(
            select(func.count())
            .select_from(Review)
            .where(Review.review_date >= today_start)
        ).scalar()
    ...
    try:
        ...
        while True:
            ...
            review_count = db.session.execute(
                select(func.count())
                .select_from(Review)
                .where(Review.review_date >= day_start, Review.review_date < day_end)
            ).scalar()
...
def get_recent_reviews():
    ...
    """
    try:
        reviews = Review.query.order_by(Review.review_date.desc()).limit(50).all()
